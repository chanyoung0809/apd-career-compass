...
2. **상단 레이서 탭**(01 김가을 ~ 07 황지혜)으로 레이서 간 이동, **오른쪽 버튼**(표지/질문·답/상담 기록/배경·가이드)으로 그 레이서의 항목 간 이동
3. 기록 입력: **상담 일시·상담 시간(30/60분)·확정 진로·한 줄 요약**, **상담 전 질문·답 1~3** (레이서가 보낸 3줄 메모), **상담 내용·고민 키워드·액션 아이템·후속 조치**
4. **우측 하단 💾 floating 저장 버튼** 클릭 → 어느 위치에서든 저장되며 `[APD1] 1on1 상담 기록.xlsx`의 개인별 시트와 '상담 로그' 시트에 동시 반영 (변경 시 주황색, 저장 성공 시 초록색으로 바뀜)
