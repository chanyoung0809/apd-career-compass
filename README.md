...
- `[APD1] 1on1 상담 기록.xlsx` — 활동 레이서 통합 양식 (**⚠️ 레이서 개인정보가 들어 있어 GitHub 저장소에는 제외** — `.gitignore` 참조. 운영 PC에만 두세요)
  - **상담 로그** 시트: 레이서별 사전 추정 진로·핵심 목표가 자동 채워져 있고, 상담 일시/내용/고민 키워드/액션 아이템/후속 조치 칸은 상담 중 직접 작성
  - **레이서 프로필** 시트: 지원동기·수료후목표 원문 (상담 직전 읽기용)
  - **개인별 시트 (`01_이름` ~)**: 1인당 1시트 — 표지·프로젝트 일정·배경 데이터·지원동기 원문·진로별 질문 가이드 + 상담 중 기록 칸. **상담할 때는 이 시트를 열어 씁니다.**
- 중도탈락·수강철회자는 **자동 제외**됩니다: `[APD1] 출석부.xlsx`의 `중도탈락`·`수강철회` 시트에서 명단을 읽어옵니다 (출석부가 Excel에 잠겨 있어도 임시 복사본으로 읽습니다. 읽기 실패 시 스크립트 안의 `EXCLUDE_FALLBACK` 명단 사용)
- 양식을 다시 뽑고 싶으면: `python tools/make_상담기록.py`
  ⚠️ 상담 메모(상담 로그·개인별 시트) 입력 후 재실행하면 메모가 사라집니다. 반드시 백업 후 실행.
