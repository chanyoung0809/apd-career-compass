...
├── 상담입력.html        ← 상담 중 기록 입력 페이지 (로컬 서버 필요)
├── 상담입력 시작.bat     ← ← 더블클릭으로 입력 페이지 열기 (★ 상담 시 이것만!)
├── PLAN.md             ← 작업 진행상황 기록
├── [APD1] 1on1 상담 기록.xlsx  ← 상담용 기록 양식 (생성물, 로컬 전용: 저장소 제외)
├── tools/
│   ├── make_상담기록.py  ← 상담 기록 양식 재생성 스크립트
│   └── 상담_서버.py      ← 입력 페이지용 로컬 서버 (엑셀 저장 담당)
└── assets/
...
### 7-1. 상담 중 기록 입력 — 상담입력 시작.bat (권장)

상담할 때는 엑셀에 직접 쓰지 않고 **입력 페이지**를 씁니다.

1. 폴더 안의 **`상담입력 시작.bat`** 더블클릭 → 브라우저 자동 오픈 (`http://localhost:8765`)
2. **좌측 상단 ☰ 메뉴** → 레이서(2depth: 레이서 → A~G 항목)에서 원하는 위치로 이동. **오른쪽 버튼**으로도 시트 섹션(A. 표지 ~ G. 상담 중 기록) 간 이동 가능
3. 기록 입력(왼쪽 폼): **상담 일시·시작 시간·소요 시간(30분/60분 스위치)·확정 진로·한 줄 요약**, **상담 전 질문·답 1~3** (레이서가 보낸 3줄 메모), **상담 내용·고민 키워드·액션 아이템·후속 조치**
4. **오른쪽 참고 컬럼**은 스크롤해도 따라와서(고정) 상담 시트를 쓰면서 **1차 프로젝트 일정·배경 데이터·지원동기 원문·질문 가이드**를 나란히 확인 가능
5. **우측 하단 💾 floating 저장 버튼** 클릭 → 어느 위치에서든 저장되며 `[APD1] 1on1 상담 기록.xlsx`의 개인별 시트와 '상담 로그' 시트에 동시 반영 (변경 시 주황색, 저장 성공 시 초록색으로 바뀜)

주의:

- **저장 전에는 엑셀 파일을 닫아 두세요.** 엑셀에서 파일을 열어 두면 저장이 막힙니다 (안내 메시지가 뜹니다).
- 서버 콘솔 창(검은 창)은 상담 동안 열어 두어야 합니다. 창을 닫으면 서버가 종료됩니다.
- 변경 후 저장하지 않고 페이지를 닫으면 경고가 뜹니다.
- 레이서 배경·지원동기 원문·질문 가이드는 페이지 하단 "펼치기"에서 읽기 전용으로 볼 수 있습니다.

### 7-2. 엑셀 양식 재생성

- `[APD1] 1on1 상담 기록.xlsx` — 활동 레이서 통합 양식 (**⚠️ 레이서 개인정보가 들어 있어 GitHub 저장소에는 제외** — `.gitignore` 참조. 운영 PC에만 두세요)
  - **상담 로그** 시트: 레이서별 사전 추정 진로·핵심 목표가 자동 채워져 있고, 상담 일시/상담 시간/내용/고민 키워드/액션 아이템/후속 조치 칸은 입력 페이지나 직접 작성
  - **레이서 프로필** 시트: 지원동기·수료후목표 원문 (상담 직전 읽기용)
  - **개인별 시트 (`01_이름` ~)**: 1인당 1시트 — 표지(상담 일시·시간)·프로젝트 일정·배경 데이터·지원동기 원문·상담 전 질문·답 1~3·진로별 질문 가이드 + 상담 중 기록 칸
