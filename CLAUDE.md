...
`tools/make_상담기록.py` → `[APD1] 1on1 상담 기록.xlsx` (총 9시트: 상담 로그 + 레이서 프로필 + **개인별 시트 7장 `NN_이름`** — 표지(상담 일시·시간)·프로젝트 일정·배경 데이터·원문·**상담 전 질문·답 1~3**·진로별 가이드·상담 중 기록. 입력 페이지가 이 라벨(A열)을 좌표로 읽고 씀).
